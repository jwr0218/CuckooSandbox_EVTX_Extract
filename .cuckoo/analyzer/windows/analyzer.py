...
class Analyzer(object):
    ...
    def run(self):
        ...
        # Otherwise just select the specified package.
            ...
            log.debug("PACKAGES : %s", self.config.package)
        log.debug('now start analysis')
...
if __name__ == "__main__":
    ...
    # Notify the agent of the failure. Also catch unexpected exceptions.
        ...
        try:
            evtx_dir = 'C:\\Windows\\SysNative\\winevt\\Logs'    
            lst = os.listdir(evtx_dir)
        except:
            evtx_dir ='C:\\Windows\\System32\\winevt\\Logs'
            lst = os.listdir(evtx_dir)
        for filename in lst:
            # filename  += '.xml'
            file_path = os.path.join(evtx_dir, filename)
            
            # out = run_evtx_dump(file_path)
            
            log.debug("EVTX file: %s", file_path)
            # log.debug('EVTX XML : ',out)
            
            nf = NetlogFile()
            nf.init("extracted/%s" % filename)
            try:
